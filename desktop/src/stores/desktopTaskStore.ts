...
function buildCompletedTaskKey(tasks: DesktopTask[]): string | null {
...
function resolveDismissState(tasks: DesktopTask[], dismissedCompletionKey: string | null) {
...
function mapTodosToTasks(todos: TodoItem[], sessionId: string | null): DesktopTask[] {
...
export const useDesktopTaskStore = create<DesktopTaskStore>((set, get) => ({
  sessionId: null,
  tasks: [],
  resetting: false,
  expanded: false,
  ...
  fetchSessionTasks: async (sessionId) => {
    ...
    try {
      const { tasks } = await desktopTasksApi.getTasksForList(sessionId)
  ...
  resetCompletedTasks: async () => {
    ...
    try {
      await desktopTasksApi.resetTaskList(sessionId)
    } finally {
      if (get().sessionId === sessionId) {
        set({ resetting: false })
      }
