 ...
 * 验证添加、激活、ycode/settings.json 同步是否正确
 * (provider env 写到 ~/.claude/ycode/settings.json，不污染原版 settings.json)
 */
...
describe('Real Provider Configs', () => {
  ...
  async function readYcodeSettings(): Promise<Record<string, unknown>> {
    const raw = await fs.readFile(path.join(tmpDir, 'ycode', 'settings.json'), 'utf-8')
  ...
  test('添加 MiniMax Provider 并激活 — 写入 ycode/settings.json', async () => {
    ...
    // 验证写入 ycode/settings.json
    const settings = await readYcodeSettings()
    ...
    console.log('✅ Provider 写入 ycode/settings.json，原版 settings.json 未被污染')
  })

  test('切换 Provider — 更新 ycode/settings.json', async () => {
    const minimax = await service.addProvider({
      presetId: 'minimax',
      name: 'MiniMax',
      baseUrl: 'https://api.minimaxi.com/anthropic',
    ...
    // 先激活 MiniMax
    await service.activateProvider(minimax.id)
    let settings = await readYcodeSettings()
    expect((settings.env as Record<string, string>).ANTHROPIC_BASE_URL).toBe('https://api.minimaxi.com/anthropic')

    // 切换到接口AI中转站
    await service.activateProvider(jiekou.id)
    settings = await readYcodeSettings()
    ...
    console.log('✅ 切换 Provider 成功，ycode/settings.json 更新正确')
  })

  test('ycode/settings.json 保留已有字段', async () => {
    // 预写一个有内容的 ycode/settings.json（模拟用户已有配置）
    await fs.mkdir(path.join(tmpDir, 'ycode'), { recursive: true })
    await fs.writeFile(
      path.join(tmpDir, 'ycode', 'settings.json'),
      JSON.stringify({
    ...
    console.log('✅ ycode/settings.json 已有字段全部保留')
  })
  ...
  test('providers.json 和 ycode/settings.json 独立于 settings.json', async () => {
    ...
    // 验证 ycode/settings.json 是 Haha 自己的
    const haha = await readYcodeSettings()
